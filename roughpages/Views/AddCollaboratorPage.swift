...
struct AddCollaboratorPage: View {
    ...
    var body: some View {
        VStack{
            ...
            Spacer()
            RoundedRectangleBtn(title: "Add Collaborators").onTapGesture {
                for email in emails {
                    ...
                    db.collection("Users").whereField("email", isEqualTo: email).limit(to: 1).getDocuments{
                        ...
                        if let err = err {
                            ...
                            let _ = documents.map{(queryDocumentSnapshot) -> UserModel in
                                ...
                                let updatePageRef = page.isPrivate ?
                                db.collection("Users").document(iAD.uid).collection("private").document(page.docId)
                                : db.collection("Pages").document(page.docId)
                                let pageData = [
                                    "collaborators": Array(emails),
                                ] as [String : Any]
                                
                                updatePageRef.setData(pageData,merge: true){
                                    error in
                                        if let error = error {
                                           ...
                                           db.collection("Users").document(uid).getDocument{(document, error) in
                                               if let document = document, document.exists {
                                                   ...
                                                   if let data = document.data() {
                                                      //print("data", data)
                                                       var favs = document["collaborations"] as? [String] ?? []
